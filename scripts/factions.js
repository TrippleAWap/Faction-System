...
export const dB = new Database("TripplesDatabase")
import { CONFIG } from "./index"
if (!dB.has("factions")) dB.set("factions", [])
if (!dB.has("playerStats")) dB.set("playerStats", [])
...
§6.f accept <faction> §f- §7Accept an invite to a faction.
...
§6.f who <name> §f- §7Tells you what faction a player is in.
§6.f top §f- §7Get the top factions.
§6.f chat §f- §7Messages you send will only be a visable to people in your faction.
...
`)
            ...
            if (args[1] == "dbv") player.sendMessage(JSON.stringify(dB.all))
            if (args[1] == "create") {
                const factionName = args[2]
                if (dB.get("factions").includes(factionName)) {
                    player.tell("§cA faction with that name already exists!")
                } else {
                    ...
                    for (const faction of dB.get("factions")) {
                        if (faction.members.includes(player.name)) {
                            player.sendMessage("§cYou are already in a faction!")
                            return
                        }
                    }
                    dB.add("factions", { name: factionName, officers: [], members: [data.sender.name], leader: data.sender.name, power: 0, home: { x: 0, y: 0, z: 0 }, enemies: [] })
            ...
            if (args[1] == "i" || args[1] == "invite") {
                ...
                if (dB.get("invites")?.find(i => i.player == target.name && i.faction.name == faction.name)) {
                ...
                dB.add("invites", { faction: faction, player: target.name })
            ...
            if (args[1] == "accept" || args[1] == "join" || args[1] == "a") {
                ...
                if (!dB.get("invites").find(i => i.player == player.name && i.faction.name == faction)) {
                    player.sendMessage("§cYou have not been invited to that faction!")
                    return
                }
                const foundFaction = dB.all.factions.find(f => f.name == faction)
                foundFaction.members.push(player.name)
                dB.get("invites").splice(dB.get("invites").indexOf(dB.get("invites").find(i => i.player == player.name && i.faction.name == faction)), 1)
                dB.save()
            ...
            if (args[1] == "disband") {
                ...
                dB.get("factions").splice(dB.get("factions").indexOf(faction), 1)
                dB.save()
            ...
            if (args[1] == "info" || args[1] == "view" || args[1] == "v") {
                ...
                const foundFaction = dB.get("factions").find(f => f.name == faction)
...
`)
            ...
            if (args[1] == "invites") {
                player.sendMessage(`§b>----------Invites----------<\n§e${dB.get("invites").filter(i => i.player == player.name).length > 0 ? dB.get("invites").filter(i => i.player == player.name).map(i => i.faction.name).join(", ") : "None"}`)
            }
            if (args[1] == "k" || args[1] == "kick") {
                if (args[2] == null) return player.sendMessage("§cPlease specify a player!")
                const faction = getFaction(player)
                if (faction == "None") {
                    player.sendMessage("§cYou are not in a faction!")
                    return
            ...
            if (args[1] == "decline" || args[1] == "deny" || args[1] == "d") {
                ...
                if (faction == "a" || "all") dB.get("invites").find(i => i.player == player.name)?.splice(0, dB.get("invites").find(i => i.player == player.name).length)
                if (!dB.get("invites").find(i => i.player == player.name && i.faction.name == faction)) {
                    player.sendMessage("§cYou have not been invited to that faction!")
                    return
                }
            ...
            if (args[1] == "sethome") {
                ...
                if (dB.get("factions").find(f => f.home != null && f.name != getFaction(player).name && Vector.distance(new Vector(f.home.x, f.home.y, f.home.z), player.location) < 275)) return player.sendMessage("§cThere is already a faction home within 275 blocks of that location!")
...
§6Power: §c${dB.get("playerStats").find(p => p.name == target.name)?.power ? dB.get("playerStats").find(p => p.name == target.name).power : 0}
...
§6KDR: §c${kd(target)}
            ...
            if (args[1] == "enemy" || args[1] == "e") {
                ...
                if (args[2] == null) return player.sendMessage("§cPlease specify a faction!")
                if (args[2] == "remove") {
                    if (args[3] == null) return player.sendMessage("§cPlease specify a faction!")
                    const target = dB.get("factions").find(f => f.name == message.split(" ").slice(3).join(" "))
                    if (target == null) {
                ...
                const target = dB.get("factions").find(f => f.name == message.split(" ").slice(2).join(" "))
                if (target == null) {
                    player.sendMessage("§cThat faction does not exist!")
                    return
                }
...
export function getFaction(player) {
    for (const faction of dB.get("factions")) {
        ...
        if (typeof player == Player && faction.members.includes(player.name)) {
            return faction
        }
    }
    return "None"
...
system.runInterval(() => {
    for (const player of world.getPlayers()) {
        ...
        if (!dB.get("playerStats").find(d => d.name == player.name)) dB.add("playerStats", { name: player.name, power: 0 })
        const foundPlayer = dB.get("playerStats").find(p => p.name == player.name)
        ...
        if (foundPlayer != dB.get("playerStats").find(p => p.name == player.name)) dB.save()

    }
    for (const faction of dB.get("factions")) {
        faction.power = 0
        for (const member of faction.members) {
            faction.power += dB.get("playerStats").find(p => p.name == member).power
        }
        for (const enemy of faction.enemies) {
            if (dB.get("factions").find(f => f.name == enemy)) continue
